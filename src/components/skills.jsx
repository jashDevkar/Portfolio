...
const Skills=({skillRef})=> {
  ...
  return (
    ...
    <section ref={skillRef} className="border-b-2 border-gray-500 pb-7">
        <header className="h-[10vh] text-center text-3xl mt-4 font-Pacifico  ">Skills</header>
        ...
        <div className="flex flex-wrap md:gap-10 mb-2 justify-center items-center md:px-7 gap-5">

            {
                myCards.map((item,index)=>(
                    ...
                    <div key={index} className="md:w-[200px] w-[170px] md:h-[240px] h-[200px] bg-[#2C2C2C] rounded-lg cursor-pointer hover:shadow-lg hover:shadow-[#D054A0] transition-shadow relative group shadow-sm" >
                        <img src={item.imagePath} className="md:w-full w-[210px] h-[160px] md:h-[80%]  rounded-lg"/>
                        <div className="flex justify-between mt-3 items-center px-2">
                         <h1 className="font-bold font-Roboto group-hover:text-[12px] md:group-hover:text-[16px]  ">{item.title}</h1>
